...
public class InstallerNodeFactory
{
    ...
    private static void handleToken(Reader reader, InstallerNodeToken token, InstallerNode node,
            InstallerNode.InstallUnit currentInstallUnit) throws InstallerNodeParseException, IOException
    {
        switch (token)
        {
            case DESC:
    ...
    public static InstallerNode readNode(Reader reader) throws InstallerNodeParseException, IOException
    {
        ...
        while (true)
        {
            ...
            {
                throw new InstallerNodeParseException(
                        "Unexpected token '" + object + "' found while looking for '" + InstallerNodeToken.NAME + "'!");
            }
        }
    ...
    private static InstallerNode readNodeSub(Reader reader) throws InstallerNodeParseException, IOException
    {
        ...
        while (true)
        {
            ...
            else if (((String) object).length() != 0)
            {
                ...
                {
                    // clear the install unit and do nothing else, causing us to ignore it until the next recognized
                    // token
                    currentInstallUnit = null;
                }
    ...
    private static String readStringUntilEndToken(Reader reader,
            InstallerNodeToken endToken) throws InstallerNodeParseException, IOException
    {
    ...
    private static List<String> readStringsUntilEndToken(Reader reader,
            InstallerNodeToken endToken) throws InstallerNodeParseException, IOException
    {
        ...
        while (true)
        {
            ...
            {
                throw new InstallerNodeParseException(
                        "Unexpected token '" + object + "' found while looking for '" + endToken + "'!");
            }
    ...
    public static void writeNode(int indent, Writer writer, InstallerNode node) throws IOException
    {
        ...
        {
            writeLine(indent, writer, InstallerNodeToken.HASH, triple.getType(), triple.getFilename(),
                    triple.getHash());
        }
